...
from krawl.normalizer.gitlab import GitLabFileHandler
...
# curl "https://gitlab.com/api/graphql" --header "Authorization: Bearer $GRAPHQL_
# TOKEN" \
# --header "Content-Type: application/json" --request POST \
# --data '{"query": "query {project(fullPath: \"OSEGermany/ohloom\") {jobs {nodes {id duration}}}}"}'
...
QUERY_PROJECTS = gql("""
query {
  projects(topics: ["OKH"], first: $batchSize) {
    nodes {
      %s
    }
    pageInfo {
...
QUERY_PROJECT = gql("""
query {
    project(fullPath: "OSEGermany/ohloom") {
        %s
    }
}
""" % (PROJECT_FIELDS))
...
class GitLabFetcher(Fetcher):
    """Fetcher for projects on GitLab.com.

    ### Finding results

    GitLab hosts more than 100 million repositories (Nov 2018) with a large
    ...
    Therefore, to accelerate the search, the code search feature of GitLab is
    used
    ...
    Searching for manifest files and downloading them from GitLab poses a couple
    of challenges that need to be addressed. Most of them stem from the rate
    limits that GitLab imposes.
    ...
    - GitLab uses rate limits for all requests made to their API. These need to
    ...
    - One of the most annoying limits is the imposed timeout for code search
      ...
      until this point will be returned. GitLab states, that the response would
      ...
      counter this, one could do the following:
        - simple search query: The more complex the search query is, the more
          ...
          384000 bytes. The size of searchable files is limited by GitLab to 384
    ...
    GitLab offers a GraphQL API that is used by the fetcher to get all projects
        ...
        "schema": {
            ...
            "access_token": {
                ...
                "meta": {
                    ...
                    "description": "Personal access token for using the GitLab API"
                }
            },
        },
    }
    ...
    def __init__(self, state_repository: FetcherStateRepository, config: Config) -> None:
        ...
        self._normalizer = ManifestNormalizer(GitLabFileHandler())
    ...
    def fetch_all(self, start_over=True) -> Generator[Project, None, None]:
        ...
        while True:
            ...
            if response.status_code == 403:
                ...
                raise FetcherError(f"failed to fetch projects from GitLab: {response.text}")
        ...
        self._state_repository.delete(self.NAME)
        log.debug("fetched %d projects from GitLab", num_fetched_projects)

    def _get_file_base_url(self, id: ProjectID) -> str:
        # get repository information
        raw_repo = self._get_repo_info(id)
        full_path = raw_repo["fullPath"]
        default_branch = raw_repo["repository"]["rootRef"]
        # base = raw_repo["webUrl"]
        ...
        return urlunparse((
            "https",
            "gitlabt.com",
            f"/{full_path}/-/raw/{default_branch}",
            None,
            None,
            None,
        ))
        # https://gitlab.com/OSEGermany/ohloom/-/raw/master/.gitignore
    ...
    def _download_manifest(self, url) -> bytes:
    # def _BLA(self, id: ProjectID) -> dict:
        ...
        # get information from GitLab
        # log.debug("requesting okh.toml for '%s/%s/%s'", id.platform, id.owner, id.repo)
        fullPath = "OSEGermany/ohloom"
        log.debug("requesting okh.toml for '%s' ..." % fullPath)
        params = {"fullPath": fullPath}
        try:
            result = self._graphql_client.execute(gql('''
query {
  project(fullPath: "OSEGermany/ohloom") {
    repository {
      blobs(ref:"master", paths: ["okh.toml"]) {
        nodes {
          rawBlob
        }
      }
            ...
            return result["project"]["repository"]["blobs"]["nodes"][0]["rawBlob"]
        except Exception as err:
            raise FetcherError(f"failed to fetch okh.toml for '{id}': {err}") from err
        finally:
            self._secondary_rate_limit.update()
    ...
    def _get_repo_info(self, id: ProjectID) -> dict:
        ...
        try:
            ...
            raise FetcherError(f"failed to fetch GitLab repository information '{id}': {e}") from e
        finally:
            self._secondary_rate_limit.update()

        # print("XXX GL.com project name: " + result)
        # print("XXX GL.com:")
        # print(result)
        # parse response data
        ...
        # )
        self._repo_cache[key] = result["project"]
